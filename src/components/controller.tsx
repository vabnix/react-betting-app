...
function Controller() {
    ...
    const [valGenerated, setValGenerated] = useState(0)
    ...
    const handlePlaceBet = async () => {
      ...
      await axios.get(url).then((res)=>{
        if(res.status == 200){
          ...
          console.error("There was some error fetching the number!")
        }
      }).catch((err)=>{
  ...
  return (
    <div className='w-full md:w-[850px] lg:w-[1200px] py-10 mx-auto px-5'>
     <Button runFunction={handlePlaceBet}/>
     <BetBox betDirection={betDirection} 
             setBetDirection={setBetDirection} 
             isLoading={isLoading} 
             valGenerated={valGenerated}
             hasWon={hasWon}/>
    </div>
  )
}
